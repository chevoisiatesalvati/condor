...
from ._shared import AGENT_OPTIONS, DEFAULT_AGENT, format_agent_llm_label
...
def _picked_model_label(sentinel_key: str, current_llm: str) -> str | None:
    ...
    if current_llm.startswith(f"{sentinel_prefix}:"):
        return f"• {format_agent_llm_label(current_llm)}"
    return None
...
def _settings_keyboard(current_llm: str) -> InlineKeyboardMarkup:
    ...
    "cursor:<id>", or "custom@venice:<model-id>"), that sentinel row matches
    and shows the pick.
    """
    keyboard = []
