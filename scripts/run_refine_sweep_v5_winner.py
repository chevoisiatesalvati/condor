#!/usr/bin/env python3
"""Refine sweep around staged v5 Phase C winner (trade-analysis driven grids).
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            f"Refine timeline sweep ({REFINE_SWEEP_VERSION}): "
            "A→B→C→D around staged v5 Phase C winner"
        )
    )
    parser.add_argument(
        "phase",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Staged v5 Phase A CSV (only for --v5-parent loading)",
    )
    parser.add_argument(
        "--phase-b-csv",
        type=Path,
        default=None,
        help="Staged v5 Phase B CSV (only for --v5-parent loading)",
    )
    parser.add_argument(
        "--phase-c-csv",
        type=Path,
        default=None,
        help="Staged v5 Phase C CSV (only for --v5-parent loading)",
    )
    parser.add_argument(
        "--refine-a-csv",
        type=Path,
...
def _load_v5_winner_parent(args: argparse.Namespace) -> tuple[str, dict]:
    ...
    print(f"v5 staged winner parent: {name}")
    return name, full
...
async def _run_refine_phase(
    ...
    *,
    ...
    parent_note = v5_parent_label or "prior refine phase"
    print(
...
async def _run_bcd(args: argparse.Namespace, v5_parent: dict) -> None:
    ...
    _name_a, parent_a = _load_refine_parent(refine_a_csv, parent_overrides=v5_parent)
...
async def _run_all(args: argparse.Namespace, v5_parent: dict, v5_label: str) -> None:
    refine_a_csv = await _run_refine_phase(
        args,
        "A",
        parent_overrides=v5_parent,
        v5_parent_label=v5_label,
    )
    args.refine_a_csv = refine_a_csv
    await _run_bcd(args, v5_parent)
...
async def main() -> int:
    ...
    try:
        v5_label, v5_parent = _load_v5_winner_parent(args)
    ...
    print(
        f"Refine sweep {REFINE_SWEEP_VERSION} | default budget ≈{total_default} configs "
        f"({', '.join(f'{p}={REFINE_MIN_CONFIGS_BY_PHASE[p]}' for p in REFINE_STAGED_PHASES)} "
        f"+ {len(REFINE_STAGED_PHASES)} anchors)"
    )

    if args.phase == "all":
        await _run_all(args, v5_parent, v5_label)
        return 0
