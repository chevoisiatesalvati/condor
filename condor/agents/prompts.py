...
- Structured layout (adapt fields to what happened):
  ...
  🔑 Executor ID: <id on its own line, full string>
...
- Learnings must specify a category: "market" or "execution".
  ...
  - execution: executor errors, schema issues, fill problems, timing, manage_executors retry patterns.
...
def _build_sizing_section(config: dict[str, Any], execution_mode: str) -> str | None:
    """Clarify notional vs leverage for position_executor sizing."""
    ...
    return (
        ...
        "Sizing: pass `notional_usd` in executor_config; Condor converts to base `amount` using live price.\n"
...
def build_tick_prompt(
    ...
    if use_cursor:
        sections.append(
            "TOOLS:\n"
            "Condor MCP stdio servers are attached to Composer for this session—call MCP tools directly by name.\n"
            'Before manage_executors(action="create"), invoke manage_executors(executor_type="...") once for schema.'
        )
    ...
    if strategy_params:
        strategy_lines = [
            "[STRATEGY CONFIG]",
            "Active strategy parameters for this session.",
        ]
    ...
    if digest_boundary and digest_interval > 0:
        sections.append(
            ...
            f"Every-{digest_interval}-tick rollup tick. If this tick is hold-only, "
            f"send ONE compact digest notification per NOTIFICATIONS rules."
        )
    if recent_decisions:
        recent_label = (
