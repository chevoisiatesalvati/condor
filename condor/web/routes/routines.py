...
async def get_routine_reports(
    ...
    # Prefer source_names when available; fall back for older in-memory modules
    # that have not been rebound after a store.py hot-reload.
    try:
        entries, total = list_reports(
            source_type="routine",
            source_names=list(allowed_names),
            limit=limit,
        )
    except TypeError:
        entries, _ = list_reports(source_type="routine", limit=10_000)
        entries = [
            entry
            for entry in entries
            if entry.get("source_name") in allowed_names
        ]
        total = len(entries)
        entries = entries[:limit]
    return {"reports": entries, "total": total}
