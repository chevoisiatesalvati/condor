...
def _merge_config_strategy_params(slug: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    from condor.agents.strategy_configs import merge_strategy_params
...
def _build_strategy_defaults(strategy) -> dict[str, Any]:
    ...
    from condor.agents.config import AgentConfig, sanitize_config_dict, strip_session_overrides

    default_config = sanitize_config_dict(
        strip_session_overrides(dict(strategy.default_config or {}))
    )
    ...
    merged.setdefault("strategy_preset", "custom")
    merged = strip_session_overrides(
        _merge_config_strategy_params(strategy.slug, merged)
    )
    preset = str(merged.get("strategy_preset") or "custom")
    if preset != "custom":
        merged = apply_agent_strategy_preset(strategy.slug, merged, preset=preset)
        merged = _merge_config_strategy_params(strategy.slug, merged)
    catalog = get_agent_strategy_preset_catalog(strategy.slug) or []
    return {
        "default_config": merged,
        "trading_context": strategy.default_trading_context or "",
        "agent_key": strategy.agent_key or "",
        "model_base_url": merged.get("model_base_url") or "",
        "strategy_presets": catalog,
    }


def _read_strategy_md(strategy) -> str:
    """Prefer private submodule agent.md, then strategy.md on disk."""
    from condor.agents.strategy_paths import resolve_agent_md_for_read

    private_md = resolve_agent_md_for_read(strategy.slug)
    if private_md is not None:
        return private_md.read_text()
    md_path = strategy.dir / "strategy.md"
    return md_path.read_text() if md_path.exists() else ""


def _write_strategy_md(strategy, content: str) -> None:
    from condor.agents.strategy_paths import agent_md_write_path

    write_path = agent_md_write_path(strategy.slug)
    if write_path.parent.exists() or write_path.parent.parent.is_dir():
        write_path.parent.mkdir(parents=True, exist_ok=True)
        write_path.write_text(content)
        return
    (strategy.dir / "strategy.md").write_text(content)


def _find_orphaned_active_sessions(run_key: str, strategy_dir: Path) -> list[int]:
...
async def get_strategy(
    ...
    defaults_payload = _build_strategy_defaults(strategy)
    ...
    return StrategyDetail(
        ...
        strategy_presets=defaults_payload.get("strategy_presets", []),
...
async def update_strategy_md(
    ...
    strategy = _get_strategy(slug, sslug)
    _write_strategy_md(strategy, req.content)
    return {"updated": True}
...
@router.get("/{slug}/strategies/{sslug}/strategy-config-schema")
async def get_strategy_config_schema(
    slug: str, sslug: str, user: WebUser = Depends(get_current_user)
):
    """Return strategy-specific parameter schema for UI forms."""
    strategy = _get_strategy(slug, sslug)
    from condor.agents.strategy_configs import get_strategy_config_schema
...
@router.get("/{slug}/strategies/{sslug}/strategy-preset-params")
async def get_strategy_preset_params(
    ...
    """Return strategy_params (+ risk hint) for a named agent preset."""
    strategy = _get_strategy(slug, sslug)
    from condor.agents.agent_presets import (
        get_agent_strategy_preset_catalog,
    ...
    if preset == "custom":
        return {"strategy_params": {}, "risk_limits": {}}
...
@router.get("/{slug}/strategies/{sslug}/defaults", response_model=StrategyDefaultsResponse)
async def get_strategy_defaults(
    slug: str, sslug: str, user: WebUser = Depends(get_current_user)
):
    """Get editable session defaults from strategy frontmatter."""
    strategy = _get_strategy(slug, sslug)
    return StrategyDefaultsResponse(**_build_strategy_defaults(strategy))


@router.put("/{slug}/strategies/{sslug}/defaults", response_model=StrategyDefaultsResponse)
async def update_strategy_defaults(
    slug: str,
    sslug: str,
    req: UpdateStrategyDefaultsRequest,
    user: WebUser = Depends(get_current_user),
):
    """Persist session defaults to strategy frontmatter."""
    store = _strategy_store()
    strategy = _get_strategy(slug, sslug)
    if req.default_config is not None:
        from condor.agents.agent_presets import apply_agent_strategy_preset
        from condor.agents.config import sanitize_config_dict, strip_session_overrides

        existing = _build_strategy_defaults(strategy)["default_config"]
        merged_defaults = strip_session_overrides(
            _deep_merge_strategy_params(existing, req.default_config)
        )
        preset = str(merged_defaults.get("strategy_preset") or "custom")
        merged_defaults = apply_agent_strategy_preset(
            strategy.slug, merged_defaults, preset=preset
        )
        merged_defaults = _merge_config_strategy_params(strategy.slug, merged_defaults)
        strategy.default_config = sanitize_config_dict(merged_defaults)
    if req.trading_context is not None:
        strategy.default_trading_context = req.trading_context
    if req.agent_key is not None:
        strategy.agent_key = req.agent_key
    if req.model_base_url is not None:
        cfg = dict(strategy.default_config)
        cfg["model_base_url"] = req.model_base_url
        strategy.default_config = cfg
...
async def get_session_executors(
    ...
    # Bot-mode: merge live positions of every base this session currently owns.
    session_nums = [
        n
...
async def start_strategy(
    ...
    """Start a strategy (new session) or resume an existing session."""
...
async def _start(agent, strategy, req: StartStrategyRequest, user_id: int) -> dict:
    ...
    from condor.agents.engine import (
    ...
    strategy_dir = strategy.data_dir
    run_key = _runkey(agent.slug, strategy.slug)
    ...
    if req.session_num is not None:
        session_dir = find_session_dir(strategy_dir, req.session_num)
        if not session_dir:
            raise HTTPException(
        ...
        agent_id = f"{run_key}_{req.session_num}"
        ...
        config_dict = apply_agent_strategy_preset(
            strategy.slug, config_dict, preset=preset
        )
        if req.config:
    ...
    config_dict = resolve_session_overrides(
        ...
        strategy_trading_context=strategy.default_trading_context,
        trading_context_override=req.trading_context,
        agent_key_override=req.agent_key,
    )

    # Web callers always act as themselves (upstream SEC): do not honor req.user_id.
    notify_chat_id = req.chat_id if req.chat_id else user_id
...
async def stop_strategy(
    ...
    if agent_id:
        from condor.agents.engine import stop_engine_by_id
...
@router.get("/{slug}/strategies/{sslug}/learnings/archive")
