...
export const api = {
  ...
  getStrategyConfigSchema: (slug: string, sslug: string) =>
    apiFetch<StrategyConfigSchema>(
      `/api/v1/agents/${encodeURIComponent(slug)}/strategies/${encodeURIComponent(sslug)}/strategy-config-schema`,
    ),

  getStrategyPresetParams: (
    slug: string,
    sslug: string,
    preset: string,
    frequencySec: number,
    ...
    apiFetch<{
      ...
      `/api/v1/agents/${encodeURIComponent(slug)}/strategies/${encodeURIComponent(sslug)}/strategy-preset-params?preset=${encodeURIComponent(preset)}&frequency_sec=${frequencySec}`,
    ),

  getStrategyDefaults: (slug: string, sslug: string) =>
    apiFetch<StrategyDefaults>(
      `/api/v1/agents/${encodeURIComponent(slug)}/strategies/${encodeURIComponent(sslug)}/defaults`,
    ),

  updateStrategyDefaults: (
    slug: string,
    sslug: string,
    data: {
      default_config?: Record<string, unknown>;
  ...
  getStrategyLearningsArchive: (slug: string, sslug: string) =>
    apiFetch<{ content: string }>(
      `/api/v1/agents/${encodeURIComponent(slug)}/strategies/${encodeURIComponent(sslug)}/learnings/archive`,
    ),
