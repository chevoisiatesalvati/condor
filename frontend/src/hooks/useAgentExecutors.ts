...
/**
 * Hook to get real-time executor data for an agent by subscribing to the shared
 * executors:{server} WS channel (ref-counted singleton) and filtering by
 * controller IDs.
 *
...
export function useAgentExecutors(
  ...
  // Read from React Query cache — useCondorWebSocket updates
  // ["executors", server, ""] on every WS message
